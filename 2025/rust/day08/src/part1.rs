...
fn kruskal(points: Vec<[i64; 3]>, k: usize) -> DisjointSet {
...
pub fn solve(parsed: Vec<[i64; 3]>, k: usize) -> u64 {
    let circuits = kruskal(parsed, k);
    let mut circuits_size = Vec::new();
    for i in 0..circuits.parent.len() {
        if circuits.parent[i] == i {
            circuits_size.push(circuits.size[i])
        }
    }
    circuits_size.sort_by(|a, b| b.cmp(a));
    circuits_size[..3].iter().map(|circ| *circ as u64).product()
}
