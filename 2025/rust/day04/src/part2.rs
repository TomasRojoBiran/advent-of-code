...
pub fn solve(mut parsed: Vec<Vec<char>>) -> u64 {
    let mut count = 0;
    loop {
        let mut count_matrix = vec![vec![0; parsed[0].len()]; parsed.len()];
        let mut marked_rolls = vec![vec![false; parsed[0].len()]; parsed.len()];
        let mut removed_rolls = false;
        for i in 0..parsed.len() {
            for j in 0..parsed[i].len() {
                if parsed[i][j] == '@' && !marked_rolls[i][j] {
